...
fn number() {
    ...
    conn.set("this_is_a_u32", 233, 0).unwrap();
    conn.incr("this_is_a_u32", 1).unwrap();
    let u: u32 = conn.get("this_is_a_u32").unwrap();
    assert!(u == 234);

    conn.set("this_is_a_i32", -23333333, 0).unwrap();
    let i: i32 = conn.get("this_is_a_i32").unwrap();
    assert!(i == -23333333);

    conn.set("this_is_a_f64", 233.333333333, 0).unwrap();
...
fn bool() {
    ...
    conn.set("this_is_a_bool", true, 0).unwrap();
