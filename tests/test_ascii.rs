...
async fn test_ascii() {
    ...
    client.flush_with_delay(1).await.unwrap();
    thread::sleep(time::Duration::from_secs(1));
    ...
    client.touch("ascii_foo", 1000).await.unwrap();

    let value: Option<String> = client.get("not_exists_key").await.unwrap();
    assert_eq!(value, None);

    client.set("ascii_pend", "y", 0).await.unwrap();
    client.append("ascii_pend", "z").await.unwrap();
    let value: Option<String> = client.get("ascii_pend").await.unwrap();
    assert_eq!(value, Some("yz".into()));
    client.prepend("ascii_pend", "x").await.unwrap();
    let value: Option<String> = client.get("ascii_pend").await.unwrap();
    assert_eq!(value, Some("xyz".into()));

    client.delete("ascii_pend").await.unwrap();
    let value: Option<String> = client.get("ascii_pend").await.unwrap();
    assert_eq!(value, None);

    assert!(client.increment("ascii_counter", 1).await.is_err());
    client.set("ascii_counter", 3, 0).await.unwrap();
    assert_eq!(client.increment("ascii_counter", 100).await.unwrap(), 103);
    assert_eq!(client.decrement("ascii_counter", 3).await.unwrap(), 100);
