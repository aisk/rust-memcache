...
mod nonblocking {
    ...
    async fn test() {
        ...
        client.set("foo", "bar", 0).await.unwrap();
        client.flush_with_delay(3).await.unwrap();
        let value: Option<String> = client.get("foo").await.unwrap();
        assert_eq!(value, Some(String::from("bar")));
        ...
        for _ in 0..1000 {
            let key = gen_random_key();
            keys.push(key.clone());
            client.set(key.as_str(), "xxx", 0).await.unwrap();
        }
    ...
    async fn issue74() {
        use memcache::{Client, CommandError, MemcacheError};
        let client = Client::connect("memcache://localhost:12346?tcp_nodelay=true").unwrap();
        client.delete("issue74").await.unwrap();
        client.add("issue74", 1, 0).await.unwrap();

        match client.add("issue74", 1, 0).await {
            Ok(_) => panic!("Should got an error!"),
            Err(e) => match e {
                MemcacheError::CommandError(e) => assert!(e == CommandError::KeyExists),
                _ => panic!("Unexpected error!"),
    ...
    async fn udp_test() {
        ...
        client.set("foo", "bar", 0).await.unwrap();
        let value = client.add("foo", "baz", 0).await;
        assert_eq!(value.is_err(), true);
        ...
        client.add("foo", "bar", 0).await.unwrap();
        let value: Option<String> = client.get("foo").await.unwrap();
        assert_eq!(value, Some(String::from("bar")));

        client.replace("foo", "baz", 0).await.unwrap();
        let value: Option<String> = client.get("foo").await.unwrap();
        assert_eq!(value, Some(String::from("baz")));

        client.append("foo", "bar").await.unwrap();
        let value: Option<String> = client.get("foo").await.unwrap();
        assert_eq!(value, Some(String::from("bazbar")));

        client.prepend("foo", "bar").await.unwrap();
        let value: Option<String> = client.get("foo").await.unwrap();
        assert_eq!(value, Some(String::from("barbazbar")));

        client.set("fooo", 0, 0).await.unwrap();
        client.increment("fooo", 1).await.unwrap();
        let value: Option<String> = client.get("fooo").await.unwrap();
        assert_eq!(value, Some(String::from("1")));

        client.decrement("fooo", 1).await.unwrap();
        let value: Option<String> = client.get("fooo").await.unwrap();
        assert_eq!(value, Some(String::from("0")));

        assert_eq!(client.touch("foooo", 123).await.unwrap(), false);
        assert_eq!(client.touch("fooo", 12345).await.unwrap(), true);

        // gets is not supported for udp
        let value: Result<std::collections::HashMap<String, String>, _> = client.gets(&["foo", "fooo"]).await;
        assert_eq!(value.is_ok(), false);
    ...
    async fn test_cas() {
        ...
        for client in clients {
            ...
            client.set("ascii_foo", "bar", 0).await.unwrap();
            let value: Option<String> = client.get("ascii_foo").await.unwrap();
            assert_eq!(value, Some("bar".into()));
